 ...
 */
public class ThermalErosionFilter implements Filter {
    ...
    public ThermalErosionFilter(int numIterations, double tAngle, double solubility){
                ...
                for (int y = 0; y < ySize; y++) {
                    ...
                    if(heightDifference>tAngle){
                        ...
                        if(abs(finalHeights[(x+xSize-1)%xSize][(y+ySize-1)%ySize] - finalHeights[x][y])  == heightDifference){
                            finalHeights[x][y] -= (heightDifference-tAngle)*solubility/2;
                            finalHeights[(x+xSize-1)%xSize][(y+ySize-1)%ySize]+=(heightDifference-tAngle)*solubility/2;
                        }
                        else if(abs(finalHeights[(x+xSize-1)%xSize][(y+ySize+1)%ySize] - finalHeights[x][y])  == heightDifference){
                            finalHeights[x][y] -= (heightDifference-tAngle)*solubility/2;
                            finalHeights[(x+xSize-1)%xSize][(y+1)%ySize]+=(heightDifference-tAngle)*solubility/2;
                        }
                        else if(abs(finalHeights[(x+xSize+1)%xSize][(y+ySize-1)%ySize] - finalHeights[x][y])  == heightDifference){
                            finalHeights[x][y] -= (heightDifference-tAngle)*solubility/2;
                            finalHeights[(x+1)%xSize][(y+ySize-1)%ySize]+=(heightDifference-tAngle)*solubility/2;
                        }
                        else if(abs(finalHeights[(x+xSize+1)%xSize][(y+ySize+1)%ySize] - finalHeights[x][y])  == heightDifference){
                            finalHeights[x][y] -= (heightDifference-tAngle)*solubility/2;
                            finalHeights[(x+1)%xSize][(y+1)%ySize]+=(heightDifference-tAngle)*solubility/2;
                        }
